  ...
  const generateMessages = () => {
    ...
    setTimeout(() => {
      const messages = [
        {
          id: 1,
          content: `'${selectedCampaign.name}' 특별 혜택을 만나보세요! ${selectedProducts.map(p => p.name).join(', ')} 제품으로 특별한 경험을 선사합니다. 지금 바로 확인하세요!`,
          tone: 'friendly',
          charCount: 89,
        },
        {
          id: 2,
          content: `안녕하세요. KT CS에서 준비한 '${selectedCampaign.name}'을 소개합니다. ${selectedProducts.map(p => p.name).join(', ')} 등 다양한 혜택을 확인해보세요.`,
          tone: 'formal',
          charCount: 76,
        },
        {
          id: 3,
          content: `'${selectedCampaign.name}'이 시작되었습니다! ${selectedProducts.map(p => p.name).join(', ')} 제품들이 특별한 가격으로 준비되어 있습니다. 놓치지 마세요!`,
          tone: 'casual',
          charCount: 82,
        },
      ];
  ...
  return (
    ...
    >
      <MessageContainer>
        ...
        <p className="text-muted mb-4">AI 기반으로 고객 세그먼트에 맞춘 마케팅 메시지를 생성합니다.</p>
            ...
            <StepContent>
              <FilterGroup>
                ...
                <Select
                  ...
                  onChange={(e) => handleFilterChange('recentPurchase', e.target.value)}
        ...
        {currentStep >= 2 && (
          <StepContainer>
            <StepHeader>
              ...
              <StepTitle active={currentStep >= 2}>필터링 실행 및 고객 수 확인</StepTitle>
            </StepHeader>
