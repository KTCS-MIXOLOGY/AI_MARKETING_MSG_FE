...
import { segmentsAPI, toneMannerAPI, campaignsAPI, productsAPI, messagesAPI, customersAPI } from "../services/api";
...
const UserMsgSeg = () => {
  ...
  // 상품 추천 불러오기 함수 (세그먼트용 - 대표 고객 ID 사용)
  const fetchProductRecommendations = async () => {
    ...
    // 세그먼트 메시지는 특정 고객 ID가 없으므로, 대표 고객 ID를 사용하거나
    // 백엔드에서 세그먼트 기반 추천을 지원하는 경우 해당 API 사용
    // 여기서는 대표 고객 ID (예: 1)를 사용하는 예시
    const representativeCustomerId = 1;

    try {
      setLoadingProductRecommendations(true);
      console.log(`캠페인 ${selectedCampaign}에 대한 상품 추천 요청 중...`);
      ...
      const messagePromises = selectedToneObjects.map(async (tone) => {
        ...
        if (response.data.success) {
          ...
          return {
            ...
            segmentFilterData: segmentFilterData, // 세그먼트 필터 데이터 저장
  ...
  const handleSaveMessage = async () => {
    ...
    try {
      ...
      const saveRequest = {
        ...
        segmentFilter: selectedToneGroup.segmentFilterData || null,
        customerId: null, // SEGMENT는 null
        campaignId: selectedCampaign,
        ...
        generationPrompt: `세그먼트 메시지 - ${selectedToneGroup.toneName} 톤`,
        aiModelUsed: "GPT-4"
      };
    ...
    >
      <Container>
        ...
        <StepContent active={currentStep === 3}>
          ...
          {/* AI 상품 추천 버튼 */}
          {!selectedProductCategory && productRecommendations.length === 0 && (
            <div style={{ marginBottom: "1.5rem", textAlign: "center" }}>
              <AIRecommendButton
                onClick={fetchProductRecommendations}
                disabled={loadingProductRecommendations || !selectedCampaign}
              >
